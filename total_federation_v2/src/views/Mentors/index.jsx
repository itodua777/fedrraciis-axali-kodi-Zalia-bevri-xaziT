    ...
    <div style={{ flex: 1, padding: "30px", backgroundColor: "#121418", color: "#e2e8f0", fontFamily: "sans-serif", display: "flex", gap: "20px", overflow: "hidden" }}>
      <div style={{ flex: selectedMentor ? 1 : 2, display: "flex", flexDirection: "column", gap: "20px", overflow: "hidden" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h2 style={{ color: "var(--color-emerald-core)", margin: 0, textShadow: "0 0 10px color-mix(in oklab, var(--color-emerald-core) 50%, transparent)" }}>მენტორები და ტექნიკური პერსონალი</h2>
          <button onClick={() => onViewChange('add_mentor')} style={{ backgroundColor: "var(--color-emerald-core)", color: "#121418", border: "none", padding: "10px 20px", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 0 15px color-mix(in oklab, var(--color-emerald-core) 50%, transparent)" }}>
        ...
        <div style={{ display: "flex", gap: "10px", backgroundColor: "rgba(15, 23, 42, 0.6)", padding: "15px", borderRadius: "12px", border: "1px solid color-mix(in oklab, var(--color-emerald-core) 10%, transparent)" }}>
          <div style={{ position: "relative", flex: 1 }}>
            ...
            <input type="text" placeholder="სახელი, გვარი, პირადი ნომერი..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ width: "100%", padding: "10px 10px 10px 35px", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid color-mix(in oklab, var(--color-emerald-core) 30%, transparent)", borderRadius: "8px", color: "#fff", outline: "none", boxSizing: "border-box" }} />
          </div>
          <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} style={{ padding: "10px", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid color-mix(in oklab, var(--color-emerald-core) 30%, transparent)", borderRadius: "8px", color: "#fff", outline: "none" }}>
          ...
          <select value={sportFilter} onChange={e => setSportFilter(e.target.value)} style={{ padding: "10px", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid color-mix(in oklab, var(--color-emerald-core) 30%, transparent)", borderRadius: "8px", color: "#fff", outline: "none" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "grid", gridTemplateColumns: selectedMentor ? "1fr" : "repeat(auto-fill, minmax(300px, 1fr))", gap: "20px", alignContent: "start" }}>
          {filteredMentors.map(m => (
            <div key={m.id} onClick={() => setSelectedMentor(m)} style={{ backgroundColor: "rgba(15, 23, 42, 0.6)", borderRadius: "12px", padding: "20px", border: `1px solid ${selectedMentor?.id === m.id ? 'var(--color-emerald-core)' : 'color-mix(in oklab, var(--color-emerald-core) 10%, transparent)'}`, cursor: "pointer", display: "flex", gap: "15px", alignItems: "center", transition: "all 0.3s", boxShadow: selectedMentor?.id === m.id ? "0 0 15px color-mix(in oklab, var(--color-emerald-core) 20%, transparent)" : "none" }}>
              <div style={{ width: "60px", height: "60px", borderRadius: "50%", overflow: "hidden", border: "2px solid var(--color-emerald-core)", flexShrink: 0 }}>
              ...
              <div style={{ backgroundColor: "color-mix(in oklab, var(--color-emerald-core) 10%, transparent)", color: "var(--color-emerald-core)", padding: "4px 8px", borderRadius: "8px", fontSize: "11px", fontWeight: "bold" }}>
                {m.category}
              </div>
            </div>
          ))}
      ...
      {selectedMentor && (
        <div style={{ flex: 1, backgroundColor: "rgba(15, 23, 42, 0.6)", border: "1px solid color-mix(in oklab, var(--color-emerald-core) 20%, transparent)", borderRadius: "12px", padding: "30px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <h3 style={{ margin: 0, color: "var(--color-emerald-core)" }}>დეტალური პროფილი</h3>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", borderBottom: "1px solid rgba(255, 255, 255, 0.1)", paddingBottom: "20px" }}>
            <div style={{ width: "100px", height: "100px", borderRadius: "50%", overflow: "hidden", border: "3px solid var(--color-emerald-core)", marginBottom: "15px" }}>
            ...
            <div style={{ color: "var(--color-emerald-core)", fontWeight: "bold" }}>{selectedMentor.status}</div>
          </div>
