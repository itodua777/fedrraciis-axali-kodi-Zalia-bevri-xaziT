...
const RouteForm = ({
  ...
  const blockTitleStyle = { color: "var(--color-emerald-core)", margin: "0 0 15px 0", fontSize: "16px", borderBottom: "1px dashed rgba(255,255,255,0.2)", paddingBottom: "5px", display: "flex", justifyContent: "space-between", alignItems: "center" };
  const availableTags = ['ნაშალი ქვები', 'კლდოვანი ქედი', 'თოვლის საფარი', 'მყინვარი', 'ყინულოვანი კედელი'];

  return (
    <div style={{ width: "450px", display: "flex", flexDirection: "column", borderRight: "1px solid color-mix(in oklab, var(--color-emerald-core) 20%, transparent)", backgroundColor: "rgba(15, 23, 42, 0.9)", padding: "20px", overflowY: "auto", zIndex: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
        <div style={{ width: "40px", height: "40px", borderRadius: "10px", border: "1px solid var(--color-emerald-core)", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "color-mix(in oklab, var(--color-emerald-core) 10%, transparent)" }}>
          <i className="fa-solid fa-mountain" style={{ color: "var(--color-emerald-core)", fontSize: "20px", textShadow: "0 0 10px var(--color-emerald-core)" }}></i>
        </div>
        <div>
          <h2 style={{ margin: 0, color: "#fff", fontSize: "20px" }}>{isEditMode ? 'რედაქტირება' : 'ახალი მარშრუტი'}</h2>
          <div style={{ fontSize: "12px", color: "var(--color-emerald-core)" }}>{isEditMode ? 'აქტიური ფორმა: Edit Mode' : 'აქტიური ფორმა: Create Mode'}</div>
        </div>
      </div>
      ...
      <h3 style={blockTitleStyle}>
        ...
        <button onClick={addWaypoint} style={{ background: "color-mix(in oklab, var(--color-emerald-core) 10%, transparent)", border: "1px solid var(--color-emerald-core)", color: "var(--color-emerald-core)", borderRadius: "4px", padding: "4px 8px", fontSize: "12px", cursor: "pointer" }}>+ დამატება</button>
      </h3>
        ...
        {formData.waypoints.map((wp, index) => (
          <div key={wp.id} style={{ backgroundColor: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.1)", padding: "10px", borderRadius: "8px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
              <span style={{ fontSize: "12px", color: "var(--color-emerald-core)" }}>Point {index + 1}</span>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "15px" }}>
        {availableTags.map(tag => (
          <div key={tag} onClick={() => toggleTag(tag)} style={{ padding: "6px 12px", borderRadius: "16px", fontSize: "12px", cursor: "pointer", border: `1px solid ${formData.terrainTags.includes(tag) ? 'var(--color-emerald-core)' : 'rgba(255,255,255,0.2)'}`, backgroundColor: formData.terrainTags.includes(tag) ? "color-mix(in oklab, var(--color-emerald-core) 10%, transparent)" : "transparent", color: formData.terrainTags.includes(tag) ? "var(--color-emerald-core)" : "rgba(255,255,255,0.5)", transition: "all 0.2s" }}>
            {tag}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: "10px", marginTop: "20px", marginBottom: "20px" }}>
        ...
        <button onClick={handleSave} style={{ flex: 2, padding: "12px", background: "var(--color-emerald-core)", color: "#121418", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 0 15px color-mix(in oklab, var(--color-emerald-core) 30%, transparent)", transition: "all 0.3s" }}>
